...
- **Scheduled task** wraps winget in `powershell.exe -NonInteractive -EncodedCommand` to inherit the auto-elevated session and suppress UAC prompts. The encoded command resolves the winget path **at runtime** using the glob `C:\Program Files\WindowsApps\Microsoft.DesktopAppInstaller_*\winget.exe` — this avoids the two known failure modes: (1) the `WindowsApps\winget.exe` alias from `Get-Command` is a reparse point that only works in interactive sessions; (2) a hardcoded versioned path breaks whenever DesktopAppInstaller updates itself.
